...
try {
    ...
    // 3. Testimonials & Categories
    $categories = db_get_all("SELECT * FROM public.categorias ORDER BY nombre ASC");
    $testimonials = db_get_all("SELECT * FROM public.testimonios ORDER BY creado_en DESC LIMIT 3");
            ...
            <div class="animate__animated animate__fadeInLeft">
                ...
                <ul style="list-style: none; padding: 0;">
                    ...
                    <li style="display: flex; align-items: center; gap: 15px; margin-bottom: 20px;">
                        ...
                        <span>Acceso prioritario al llegar.</span>
                    </li>
                </ul>
            </div>
                ...
                <form action="procesar_reserva.php" method="POST">
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                        ...
                        <div class="form-group">
                            ...
                            <input type="tel" name="telefono" class="form-control" required placeholder="900 000 000">
                        </div>
                        <div class="form-group">
...
<section id="blog">
    <div class="container">
        ...
        <?php if (isset($_GET['testimonio']) && $_GET['testimonio'] === 'success'): ?>
            <div style="background: rgba(0, 255, 136, 0.1); color: #00ff88; padding: 15px; border-left: 4px solid #00ff88; margin-bottom: 30px; font-family: monospace; letter-spacing: 1px;">
                > STATUS_SYS: Testimonio registrado. ¡Gracias por compartir tu experiencia!
            </div>
        <?php endif; ?>
        
        <div class="menu-grid">
            <?php if (empty($testimonials)): ?>
                <div class="menu-card glass" style="padding: 40px; text-align: center;">
                    ...
                    <p style="font-style: italic; margin-bottom: 20px;">"El mejor pollo que he probado en años."</p>
                    <h4 style="color: var(--primary);">JUAN PÉREZ</h4>
                </div>
            <?php else: ?>
                    ...
                    <div class="menu-card glass" style="padding: 40px; text-align: center; border-image: linear-gradient(to bottom right, var(--primary), transparent) 1;">
                        <div style="color: var(--secondary); margin-bottom: 20px;">
                            <?php for($i=1; $i<=5; $i++): ?>
                                ...
                                <?php else: ?>
                                    <i data-lucide="star" style="opacity: 0.3;"></i>
                                <?php endif; ?>
                            <?php endfor; ?>
                        </div>
                        <p style="font-style: italic; margin-bottom: 20px; color: var(--text-primary);">"<?= htmlspecialchars($t['contenido']) ?>"</p>
                        <h4 class="neon-glow" style="color: var(--primary); letter-spacing: 3px;"><?= strtoupper(htmlspecialchars($t['nombre'])) ?></h4>
                    </div>
                <?php endforeach; ?>
            <?php endif; ?>
        </div>
