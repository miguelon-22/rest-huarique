...
if (isset($_GET['status']) && isset($_GET['id'])) {
    ...
    if ($status === 'completado') {
        try {
            ...
            $p = db_get_one("SELECT p.*, c.nombre, c.email as correo FROM public.pedidos p LEFT JOIN public.clientes c ON p.cliente_id = c.id WHERE p.id = ?", [$id]);
            if ($p && !empty($p['correo'])) {
                $items = db_get_all("SELECT * FROM public.detalle_pedidos WHERE pedido_id = ?", [$id]);
                $order_data = [
                    'nombre' => $p['nombre'],
                    'numero_pedido' => $p['numero_pedido'],
                    'monto' => $p['precio_total']
                ];
                $body = get_order_email_template($order_data, $items);
                send_huarique_email($p['correo'], "TU RECIBO DE COMPRA - {$p['numero_pedido']}", $body);
                $msg .= ' Se ha enviado el recibo al cliente.';
            }
        } catch (Exception $e) {
            error_log("Email sending failed for order $id: " . $e->getMessage());
        }
    }
}

$pedidos = db_get_all("SELECT p.*, c.nombre as cliente_nombre, c.dni as cliente_dni, c.email as cliente_correo, c.telefono as cliente_telefono, c.direccion as cliente_direccion FROM public.pedidos p LEFT JOIN public.clientes c ON p.cliente_id = c.id ORDER BY p.creado_en DESC");
