...
if (isset($_POST['save_testimonio'])) {
    if ($_SESSION['admin_role'] !== 'admin') {
        ...
        if (!empty($nombre) && !empty($contenido)) {
            db_execute(
                "INSERT INTO public.testimonios (nombre, correo, contenido) VALUES (?, ?, ?)",
            ...
            <tbody>
                <?php foreach ($testimonios as $t): ?>
                    <tr>
                        ...
                        <td style="font-size:0.75rem; color: var(--accent); white-space: nowrap;">
                            <?= !empty($t['correo']) ? htmlspecialchars($t['correo']) : '<span style="color:var(--text-secondary);">—</span>' ?>
                        </td>
                        <td>
